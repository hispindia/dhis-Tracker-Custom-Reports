select '0. Overall Performance' title,cat, sum(e1) e1,sum(e2) e2,sum(e3) e3,sum(e4) e4,sum(e5) e5
from
	(
	select cat,
	...
	from 
		...
		case when (sum(v)/350)*100 > 75 then '# of Enterprise are in Category A' 
		     when (sum(v)/350)*100 <= 75 and ((sum(v)/350)*100) >= 50 then '# of Enterprise are in Category B' 
		     when (sum(v)/350)*100 < 50 then '# of Enterprise are in Category C' 
		     end cat
		from
			(
...
select '1. Entrepreneur Background' title,cat, sum(e1) e1,sum(e2) e2,sum(e3) e3,sum(e4) e4,sum(e5) e5
from
	(
	select cat,
	case when etype=1 then 1 end e1,case when etype=2 then 1 end e2,case when etype=3 then 1 end e3,
	...
	from 
		(
		select 	row_number() over(PARTITION BY ou order by ou,psi) etype, ou,psi,
		case when ((sum(v)/60)*20)> 15 then '# of Enterprise are in Category A' 
		     when ((sum(v)/60)*20) <= 15 and ((sum(v)/60)*20) >= 10 then '# of Enterprise are in Category B' 
		     when ((sum(v)/60)*20) < 10 then '# of Enterprise are in Category C' 
		     end cat
		from
			(
			select td.programstageinstanceid psi, td.dataelementid de, td.value::FLOAT v,psi.organisationunitid ou,os.level
...
select '2. Professional cum Technical Skill' title,cat, sum(e1) e1,sum(e2) e2,sum(e3) e3,sum(e4) e4,sum(e5) e5
from
	(
	select cat,
	case when etype=1 then 1 end e1,case when etype=2 then 1 end e2,case when etype=3 then 1 end e3,
	...
	from 
		(
		select 	row_number() over(PARTITION BY ou order by ou,psi) etype, ou,psi,
		case when ((sum(v)/80)*20)> 15 then '# of Enterprise are in Category A' 
		     when ((sum(v)/80)*20) <= 15 and ((sum(v)/80)*20) >= 10 then '# of Enterprise are in Category B' 
		     when ((sum(v)/80)*20) < 10 then '# of Enterprise are in Category C' 
		     end cat
		from
			(
			select td.programstageinstanceid psi, td.dataelementid de, td.value::FLOAT v,psi.organisationunitid ou,os.level
...
select '3. Documentation Requirement' title,cat, sum(e1) e1,sum(e2) e2,sum(e3) e3,sum(e4) e4,sum(e5) e5
from
	(
	select cat,
	case when etype=1 then 1 end e1,case when etype=2 then 1 end e2,case when etype=3 then 1 end e3,
	...
	from 
		(
		select 	row_number() over(PARTITION BY ou order by ou,psi) etype, ou,psi,
		case when ((sum(v)/50)*15)> 11.25 then '# of Enterprise are in Category A' 
		     when ((sum(v)/50)*15) <= 11.25 and ((sum(v)/50)*15) >= 7.5 then '# of Enterprise are in Category B' 
		     when ((sum(v)/50)*15) < 7.5 then '# of Enterprise are in Category C' 
		     end cat
		from
			(
			select td.programstageinstanceid psi, td.dataelementid de, td.value::FLOAT v,psi.organisationunitid ou,os.level
...
select '4. Market Linkages' title,cat, sum(e1) e1,sum(e2) e2,sum(e3) e3,sum(e4) e4,sum(e5) e5
from
	(
	select cat,
	case when etype=1 then 1 end e1,case when etype=2 then 1 end e2,case when etype=3 then 1 end e3,
	...
	from 
		(
		select 	row_number() over(PARTITION BY ou order by ou,psi) etype, ou,psi,
		case when ((sum(v)/70)*15)> 11.25 then '# of Enterprise are in Category A' 
		     when ((sum(v)/70)*15) <= 11.25 and ((sum(v)/70)*15) >= 7.5 then '# of Enterprise are in Category B' 
		     when ((sum(v)/70)*15) < 7.5 then '# of Enterprise are in Category C' 
		     end cat
		from
			(
			select td.programstageinstanceid psi, td.dataelementid de, td.value::FLOAT v,psi.organisationunitid ou,os.level
...
select '5. Business Performance' title,cat, sum(e1) e1,sum(e2) e2,sum(e3) e3,sum(e4) e4,sum(e5) e5
from
	(
	select cat,
	case when etype=1 then 1 end e1,case when etype=2 then 1 end e2,case when etype=3 then 1 end e3,
	...
	from 
		(
		select 	row_number() over(PARTITION BY ou order by ou,psi) etype, ou,psi,
		case when ((sum(v)/90)*30)> 22.5 then '# of Enterprise are in Category A' 
		     when ((sum(v)/90)*30) <= 22.5 and ((sum(v)/90)*30) >= 15 then '# of Enterprise are in Category B' 
		     when ((sum(v)/90)*30) < 15 then '# of Enterprise are in Category C' 
		     end cat
		from
			(
			select td.programstageinstanceid psi, td.dataelementid de, td.value::FLOAT v,psi.organisationunitid ou,os.level
